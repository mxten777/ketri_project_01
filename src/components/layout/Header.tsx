  ...
  const menuItems: MenuItem[] = [
    ...
    },
    {
      ...
      children: [
        ...
        { label: '수거기준 및 실행방법', path: '/dialysis-water/collection-standards' },
        { label: '채수방법 및 시료채수 위치', path: '/dialysis-water/sampling-methods' },
      ]
    },
    {
      label: '실내공기질 측정',
      ...
      children: [
        ...
        { label: '실내공기질 측정 견적 요청', path: '/indoor-air-quality/quote-request' },
        { label: '실내공기질 성적서 조회', path: '/indoor-air-quality/certificate-lookup' },
      ]
    },
    {
      label: '석면조사·분석',
          ...
          <nav className="hidden 2xl:flex items-center space-x-1">
            {menuItems.map((item) => (
              <div key={item.path} className="relative group">
                <Link
                  to={item.path}
                  className={`px-2.5 py-2 rounded-lg text-xs font-medium transition-colors duration-200 flex items-center space-x-1 whitespace-nowrap
            ...
            {currentUser ? (
              <div className="relative">
                <button
                  ...
                  className="hidden md:flex items-center space-x-2 px-4 py-2 rounded-lg text-neutral-600 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors duration-200"
                >
                  <User className="w-5 h-5" />
                  <span className="text-sm">{userData?.displayName || '사용자'}</span>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="xl:hidden border-t border-neutral-200 dark:border-neutral-800"
          >
