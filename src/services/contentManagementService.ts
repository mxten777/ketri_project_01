...
class ContentManagementService {
  ...
  // 컨텐츠 생성
  async createContent(contentData: Omit<ContentItem, 'id' | 'createdAt' | 'updatedAt' | 'viewCount' | 'likes' | 'comments'>): Promise<string> {
    try {
      ...
      // 활동 로그 추가
      await this.addActivityLog('created', contentData.title, contentData.author);
  ...
  async updateContent(id: string, updates: Partial<ContentItem>): Promise<void> {
    try {
      ...
      // 활동 로그 추가
      await this.addActivityLog('updated', updates.title || 'Unknown', updates.author || 'Unknown');
    } catch (error) {
  ...
  // 컨텐츠 상태 변경 (일괄 처리)
  async updateContentStatus(ids: string[], status: 'draft' | 'published' | 'archived'): Promise<void> {
    try {
      ...
      // 활동 로그 추가
      await this.addActivityLog('bulk_status_update', `${ids.length} items to ${status}`, 'Admin');
    } catch (error) {
  ...
  async getContentStatistics(): Promise<ContentStatistics> {
    try {
      ...
      querySnapshot.forEach(doc => {
        ...
        // 카테고리별 집계
        contentByCategory[data.category] = (contentByCategory[data.category] || 0) + 1;
      });
  ...
  // 활동 로그 추가
  private async addActivityLog(action: string, contentTitle: string, authorName: string): Promise<void> {
    try {
